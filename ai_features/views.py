...
from .models import CareerRoadmap, CVProfile, MockInterviewSession, MockInterviewAnswer
...
class MockInterviewStartView(APIView):
    ...
    Body: { "role": "Backend Developer", "level": "Mid", "job_description": "..." }
    Generates questions and saves a new session.
    """
    permission_classes = [permissions.IsAuthenticated]
...
class MockInterviewSubmitView(APIView):
    ...
    Body: {
      "session_id": 1,
      "answers": [
        { "question_id": 1, "answer": "My answer here..." },
        { "question_id": 2, "answer": "My answer here..." }
      ]
    }
    AI grades each answer and saves results.
    """
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request):
        ...
        for ans in answers:
            ...
            q_type   = q_obj.get('type', 'technical')
            qa_pairs.append({
                'question_id': qid,
                'question':    question,
                'type':        q_type,
...
class MockInterviewHistoryView(APIView):
    ...
    Returns all past interview sessions for the current user.
    """
    permission_classes = [permissions.IsAuthenticated]
...
class MockInterviewDetailView(APIView):
    ...
    Returns a specific session with all graded answers.
    """
    permission_classes = [permissions.IsAuthenticated]
